...
#define BAT_CHARGED_PIN PIN_PA11
...
#define SYSTEM_TC_PERIOD_VALUE                                                 \
  111 // Tuned for 1.99 -- 2.002 ms (1ms * 2) Period
...
void sys_tc_callback(
    struct tc_module *const
        module_inst); // system timer callback function to set up system ticks
void startup_default_pin_state(void); // system startup GPIO pin set ups
void startup_sys_configs(
    void); // Function calls to set up start up config functions
